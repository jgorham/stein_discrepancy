...
# This function runs the approximate gibbs smapler.
#
# Inputs:
# d                - the MRF target distribution to approximate a sample from
# theta0           - the starting point for the sampler
# numiter          - the number of iterations to run the sampler (i.e. number of points)
# numlikelihood    - the number of likelihood evaluations to make before halting
# batchsize        - the number of observations to evaluate before doing the significance test
...
# Returns:
# (samples, number of gradient evaluations)
function runapproxgibbs(d::SteinMRF,
                        theta0::Array{Int64};
                        numiter=Inf,
                        numlikelihood=Inf,
                        batchsize=10,
    ...
    p = numdimensions(d)
    # Number of potential functions per coordinate
    L = binomial(p-1,2)
    # Iteration counter, increments after each full scan
    t = 0
    thetas = Array(Int64, 0, p)
    theta_t = theta0
    numlikelihoods = Int64[]
    cumlikelihoods = 0

    # For each coordinate i, compute the (p-1 choose 2) choices of (j,k)
    # such that i, j, k are all distinct and j < k
    good_indices = Array(Int64, L, p)
    for i = 1:p
        a = 0
        for j = setdiff(1:p, i)
            for k = setdiff((j+1):p, i)
                a += 1
                good_indices[a, i] = sub2ind((p,p),j,k)
            end
        end
    end

    while t < numiter && cumlikelihoods < numlikelihood
        # Inner loop is over the coordinates of x; we draw a new minibatch
        # of potentials for each coordinate update
        for i = 1:p
            significancelevel = 1.0
            currbatchsize = 0
            ldiffmean = 0.0
            perm = shuffle(good_indices[:,i])
            u = rand()
            mu0 = log(u/(1-u))/L
            ldiffs = zeros(L)
            while (currbatchsize < L) && (significancelevel > epsilon)
                oldbatchsize = currbatchsize
                currbatchsize = min(currbatchsize + batchsize, L)
                for ii = (oldbatchsize+1):currbatchsize
                    j, k = ind2sub((p,p), perm[ii])
                    ldiffs[ii] = d.logpsi[i,j,k,1+1,theta_t[j]+1,theta_t[k]+1] -
                                 d.logpsi[i,j,k,0+1,theta_t[j]+1,theta_t[k]+1]
                end
                ldiffmean = mean(ldiffs[1:currbatchsize])
                lstd = std(ldiffs[1:currbatchsize]) *
            ...
            cumlikelihoods += currbatchsize
            theta_t[i] = ldiffmean > mu0 ? 1 : 0
        end

        # After each full scan through the coordinates, save sample and
        # cumulative number of likelihood evaluations
        t += 1
        thetas = [thetas; theta_t']
        push!(numlikelihoods, cumlikelihoods)
    end
