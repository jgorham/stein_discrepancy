...
get_epsilon_labeller <- function(dec.places=0, prefix="", variable="epsilon") {
    ...
    function(values) {
        ...
        sapply(sci.values, function(sci.value) {
            if (sci.value == "0e+00") {
                ...
                numparts <- strsplit(sci.value, "e")[[1]]
                base <- numparts[1]
                exponent <- as.char(as.num(numparts[2]))
                if (base == "1") {
                    sci.value <- paste0("10^{", exponent, "}")
                } else {
                    sci.value <- paste0(base, " %*% 10^{", exponent, "}")
                }
            }
            if (variable == "") {
                paste0(prefix, sci.value)
            } else {
                paste0(prefix, variable, " == ", sci.value)
            }
        })
    }
}
