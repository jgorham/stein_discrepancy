...
class MultiquadricQuadraticTest:
    def __init__(self, grad_log_prob, c=1.0, beta=-0.5, grad_log_prob_multiple=None):
        # beta is exponent
        self.beta = beta
        # c^2 goes inside the parens
        self.c2 = c ** 2
        self.grad = grad_log_prob
    ...
    def k_multiple_dim(self, X):
        ...
        assert X.shape[0] > X.shape[1]
        sq_dists = squareform(pdist(X, 'sqeuclidean'))
        K = np.power(self.c2 + sq_dists, self.beta)
        return K

    def g1k_multiple_dim(self,X,K,dim):
        X_dim = X[:,dim]
        assert X_dim.ndim == 1
        X_col = X_dim.reshape(len(X_dim), 1)
        X_row = X_dim.reshape(1,len(X_dim))
        differences = X_col - X_row

        B = np.power(K, (self.beta - 1.0) / self.beta)

        return 2.0 * self.beta * (differences * B)
    ...
    def gk_multiple_dim(self,X,K,dim):
        ...
        B = np.power(K, (self.beta - 1.0) / self.beta)
        B2 = np.power(K, (self.beta - 2.0) / self.beta)

        return -2.0 * self.beta * (
            B + 2 * (self.beta - 1.0) * sq_differences * B2
        )
