function output = kernel_herding(gm_model, N, options)
% function output = kernel_herding(gm_model, N, options)
%  
...
%           DEACTIVATED HERE TO SAVE TIME --> remove computation of ||mu_p||^2
...
%     %%output.f_proj_stores -> track objective value if a projection was used... [REMOVED]
...
% %% computing  ||mu_p||^2:
% --- REMOVED HERE TO ACCELERATE CODE
...
for t = 0:(T-1)
    ...
    if step_choice == 0 || step_choice == 3
        % line-search:
        if gap<eps
            if 0 %fvalue < eps % then we have basically converged
                % [NEED TO THINK MORE ABOUT WHAT TO DO when not computing
                % fvalue...]
