...
using SteinDiscrepancy: LangevinKernelResult
...
#  subsamplescoresize - if not 0, the procedure will subsample
#    this many data observations in the SteinDistribution to estimate
#    the score function rather than sum over all the observations.
...
function langevin_kernel_discrepancy(points::Array{Float64},
    ...
    gradlogdensities = compute_gradlogdensities(target,
                                                points;
...
# used in the final kernel evaluations. The subsamplescoresize
# (if not 0) specifies how many observations to sample from
# the target in order to compute its gradient. This obviously
# assumes that the target factors as a product over these observations.
function compute_gradlogdensities(target::SteinDistribution,
                                  points::Array{Float64,2};
                                  subsamplescoresize=0)
    n = size(points, 1)
    if !isa(target, SteinPosterior) || subsamplescoresize == 0
        gradlogdensities = @parallel (vcat) for i=1:n
            gradlogdensity(target, points[i:i,:])
        end
    else
        seed = Base.Random.RANDOM_SEED
        N = numdatapoints(target)
        gradlogdensities = @parallel (vcat) for i=1:n
            srand(hash("$seed:$i"))
            gradlogdensity(target, points[i:i,:];
                           idx=sample(1:N, subsamplescoresize, replace=false))
        end
    end
    gradlogdensities
end
